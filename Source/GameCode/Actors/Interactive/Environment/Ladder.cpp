...
void ALadder::OnConstruction(const FTransform& Transform)
{
	...
	if (IsValid(LeftRailMesh))
	{
		...
		{
			LeftRailMeshComponent->SetRelativeScale3D(FVector(1.0, 1.0, LadderHeight / MeshHeight));
		}
	}
	...
	if (IsValid(RightRailMesh))
	{
		...
		if (!FMath::IsNearlyZero(MeshHeight))
		{
			RightRailMeshComponent->SetRelativeScale3D(FVector(1.0, 1.0, LadderHeight / MeshHeight));
		}
	}
	...
	TopInteractionVolume->SetRelativeLocation(FVector(-TopBoxExtent.X + BoxDepthExtent*2, 0.0f, LadderHeight + TopBoxExtent.Z));
}
