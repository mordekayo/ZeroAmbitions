...
#include "UI/Widgets/InventoryViewWidget.h"
...
void FInventorySlot::UnbindInventorySlotUpdate() const
{
	OnInventorySlotUpdate.Unbind();
}

void FInventorySlot::UpdateSlotState() const
{
...
void UCharacterInventoryComponent::CloseViewInventory() const
{
	if(InventoryViewWidget->IsVisible())
	{
...
{
	const FInventorySlot* ItemSlot = FindItemSlot(ItemID);
	if(ItemSlot != nullptr)
	{
...
TArray<FInventorySlot> UCharacterInventoryComponent::GetAllItemCopy() const
{
